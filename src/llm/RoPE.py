...
class RotaryPositionalEmbedding(nn.Module):
    def __init__(self, theta: float, d_k: int, max_seq_len: int, device=None):
        ...
        # (max_seq_len,)
        position = torch.arange(self.max_seq_len, dtype=torch.float32, device=self.device)
    ...
    def forward(
        ...
        if token_positions.dtype not in (torch.int8, torch.int16, torch.int32, torch.int64, torch.uint8):
            raise TypeError(f"token_positions.dtype ({token_positions.dtype}) must be an integer type")
        ...
        flat_pos = token_positions.reshape(-1).to(device=self.cos_cached.device, dtype=torch.long)
        cos = self.cos_cached.index_select(0, flat_pos).reshape(*token_positions.shape, self.d_k)
        sin = self.sin_cached.index_select(0, flat_pos).reshape(*token_positions.shape, self.d_k)
